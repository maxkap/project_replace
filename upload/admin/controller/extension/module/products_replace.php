...
class ControllerExtensionModuleProductsReplace extends Controller {
    ...
    public function __construct($registry) {
        ...
        $this->token_name = $this->session->data[$this->token_name]?$this->token_name:'user_token';
    }
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('module/products_replace', $this->token_name.'=' . $this->session->data[$this->token_name], true)
        );

        $data['start'] = $this->url->link('module/products_replace/start', $this->token_name.'=' . $this->session->data[$this->token_name], true);
    ...
    public function start() {
        ...
        if (isset($this->request->post['categories']) && isset($this->request->post['to_category_id'])) {
            ...
            $emulate = (isset($this->request->post['emulate'])) ? 1 : 0;

            $this->model_module_products_replace->startReplace($categories, $category_to, $replace_to, $replace_from, $emulate);
        ...
        $this->response->redirect($this->url->link('module/products_replace', $this->token_name.'=' . $this->session->data[$this->token_name], true));
    }
